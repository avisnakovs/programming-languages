...
puts 'Point eval_prog should return self' if a.eval_prog([]) != a
puts 'Point preprocess_prog should return self' if a.preprocess_prog != a
...
puts 'Line eval_prog should return self' if b.eval_prog([]) != b
puts 'Line preprocess_prog should return self' if b.preprocess_prog != b
...
puts 'Var preprocess_prog should return self' if v.preprocess_prog != v
...
l = Let.new('a', LineSegment.new(-ONE, -TWO, THREE, FOUR),
            Intersect.new(Var.new('a'), LineSegment.new(THREE, FOUR, -ONE, -TWO)))
l1 = l.preprocess_prog.eval_prog([])
puts 'Let eval_prog should evaluate e2 after adding [s, e1] to the environment' unless (l1.x == THREE) && (l1.y == FOUR)
